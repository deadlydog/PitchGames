...
namespace CS_827_Project_Source_Code
{
    class CPong : CGame
    {
        ...
        int miPitchMeterIndicatorYPosition; // Used to tell where the Players current pitch is
        ...
        public CPong()
        {
            ...
            {
                // Pre-load images
                mcPaddleImage = Image.FromFile("../../Data/Images/PongPaddle.png");
                mcBallImage = Image.FromFile("../../Data/Images/PongBall.png");

                // Pre-load sounds
                CFMOD.CheckResult(CFMOD.GetSystem().createSound("../../Data/Sounds/PongBallBounce.wav", FMOD.MODE.DEFAULT, ref msSoundBallBounce), "FMOD Create Sound Error");
                CFMOD.CheckResult(CFMOD.GetSystem().createSound("../../Data/Sounds/PongBallDied.wav", FMOD.MODE.DEFAULT, ref msSoundBallDied), "FMOD Create Sound Error");
                CFMOD.CheckResult(CFMOD.GetSystem().createSound("../../Data/Sounds/PongGameOver.wav", FMOD.MODE.DEFAULT, ref msSoundGameOver), "FMOD Create Sound Error");
            }
            catch (Exception e)
            {
        ...
        private void UpdateGameObjects(float fTimeSinceLastUpdateInSeconds, Graphics cBackBuffer)
        {
            ...
            // Move the Paddle based on Sound Input
            if (CSoundInput.Instance().fPitchLastFrame >= CSoundInput.Instance().iLowerPitchRange &&
                CSoundInput.Instance().fPitch >= CSoundInput.Instance().iLowerPitchRange)
            {
                // Calculate what Percent of force should be used to move the Paddle
                float fPercentToMove = (CSoundInput.Instance().fPitch - (float)CSoundInput.Instance().iMiddlePitchRange) / (float)(CSoundInput.Instance().iUpperPitchRange - CSoundInput.Instance().iMiddlePitchRange);
            ...
            else if (rBallCollisionRect.IntersectsWith(mrLeftWall))
            {
                ...
                if (miLives == 0)
                {
                    ...
                    if (CProfiles.Instance().mcCurrentProfile.iScorePong < miScore)
                    {
                        // Save the Players new High Score
                        CProfiles.Instance().mcCurrentProfile.iScorePong = miScore;
